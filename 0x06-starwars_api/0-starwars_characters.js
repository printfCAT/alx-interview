...
function getCharacters(movieId) {
  return new Promise((resolve, reject) => {
    request({ uri: `https://swapi-api.alx-tools.com/api/films/${movieId}`, json: true },
      (filmError, filmResponse, filmBody) => {
        if (filmError) {
          ...
          const characterPromises = characters.map(
            (characterUrl) => new Promise((characterResolve, characterReject) => {
              request({ uri: characterUrl, json: true },
